...
public class Enchantments {
	...
	public static Enchantments getEnchantmentsById(int id) {
		for (int i = 0; i < enchantments.length; i++) {
			if (enchantments[i].getId() == id)
				return enchantments[i];
		}
		return null;
	}

	public static Enchantments getEnchantmentsByName(String name) {
		for (int i = 0; i < enchantments.length; i++) {
			if (enchantments[i].getName().toLowerCase().equals(name.toLowerCase()))
				return enchantments[i];
		}
		return null;
	}

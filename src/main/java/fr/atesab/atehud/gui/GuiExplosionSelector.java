...
public class GuiExplosionSelector extends GuiScreen {
	...
	private GuiFireWorkFactory Last;

	private int[] colors = { -1, -1, -1, -1, -1, -1, -1 }, fadeColors = { -1, -1, -1, -1, -1, -1, -1 };

	private int ExpIndex, nextGuiId = 0, Type = 0, nbColor = 0, nbFadeColor = 0, maxColor = colors.length,
			maxFadeColor = fadeColors.length;

	private GuiButton addColor, addFadeColor;

	private List color = new ArrayList(), fadeColor = new ArrayList();

	public GuiExplosionSelector(GuiFireWorkFactory last, int expIndex, Explosion exp) {
		super();
		Last = last;
		ExpIndex = expIndex;
		boolean trl = false;
	...
	public void actionPerformed(GuiButton button) {
		...
		if (button == buttonDone) {
			...
			for (int i = 0; i < getFadeColorSize(); i++) {
				System.out.println(doneFadeColors[i]);
			}
			mc.displayGuiScreen(Last);
		}
		if (button == Trail) {
		...
		if (button.id == 9) {
			if (getColorSize() < colors.length) {
				if (buffer) {
					buffer = false;
					addColor(fr.atesab.atehud.superclass.Colors.WHITE);
				}
			}
		}
		if (button.id == 10) {
			if (getFadeColorSize() < fadeColors.length) {
				if (buffer) {
					buffer = false;
					addFadeColor(fr.atesab.atehud.superclass.Colors.WHITE);
				}
			}
		}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		fontRendererObj.drawString(LanguageRegistry.instance().getStringLocalization("gui.act.type"),
				width / 2 - 200 + 100
						- fontRendererObj
								.getStringWidth(LanguageRegistry.instance().getStringLocalization("gui.act.type")) / 2,
				height / 2 - 9 - fontRendererObj.FONT_HEIGHT, fr.atesab.atehud.superclass.Colors.GOLD);
		...
		for (int i = 0; i < getColorSize(); i++) {
			...
			buttonList.add(c);
			buttonList.add(new GuiButton(16 + a + colors.length, width / 2 + 85, height / 2 - 71 + 21 * a, 20, 20,
					"\u00a7c-"));
			a++;
		}
		if (addColor != null)
		...
		fontRendererObj.drawString(s, width / 2 - fontRendererObj.getStringWidth(s) / 2, 40,
				fr.atesab.atehud.superclass.Colors.GOLD);

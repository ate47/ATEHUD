...
public class GuiAttributeSelector extends GuiScreen {
	...
	private GuiTextField[] amountList = new GuiTextField[] {}, UUIDLeast = new GuiTextField[] {},
			UUIDMost = new GuiTextField[] {};
	...
	public void actionPerformed(GuiButton button) {
		...
		if (button == bdone) {
			Attribute[] attriList = new Attribute[list.length];
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for (int i = 0; i < amountList.length; i++) {
			...
			fontRendererObj.drawString(str, width / 2 - fontRendererObj.getStringWidth(str) / 2, 25 + i * elementSize,
					outColor);
		}
		String str = "UUID", str2 = "Least = ", str3 = "Most = ";
		for (int i = 0; i < amountList.length; i++) {
			amountList[i].drawTextBox();
			GuiUtils.drawRightString(fontRendererObj, I18n.format("gui.act.itemfactory.attribute.amount"),
					width / 2 - 80, 25 + fontRendererObj.FONT_HEIGHT + 1 + i * elementSize, 20, Colors.GOLD);

		}
		if (isAdvanced)
			for (int i = 0; i < UUIDLeast.length; i++) {
				fontRendererObj.drawString(str, width / 2 - fontRendererObj.getStringWidth(str) / 2,
						25 + i * elementSize + fontRendererObj.FONT_HEIGHT + 1 + 20, Colors.RED);
				GuiUtils.drawRightString(fontRendererObj, str2, width / 2 - 80,
						45 + 2 * (fontRendererObj.FONT_HEIGHT + 1) + i * elementSize, 20, Colors.RED);
				GuiUtils.drawRightString(fontRendererObj, str3, width / 2 + 125,
						45 + 2 * (fontRendererObj.FONT_HEIGHT + 1) + i * elementSize, 20, Colors.RED);
	...
	public void initGui() {
		...
		int tfid = 0;
		elementSize = 20 + 2 + fontRendererObj.FONT_HEIGHT + adv * (20 + 2 + fontRendererObj.FONT_HEIGHT);
		amountList = new GuiTextField[list.length];
		for (int i = 0; i < amountList.length; i++) {
			amountList[i] = new GuiTextField(tfid, fontRendererObj, width / 2 - 80,
					2 + 25 + fontRendererObj.FONT_HEIGHT + 1 + i * elementSize, 75, 16);
			tfid++;
		}
		operation0List = new GuiButton[list.length];
		for (int i = 0; i < operation0List.length; i++) {
			operation0List[i] = new GuiButton(tfid, width / 2, 25 + fontRendererObj.FONT_HEIGHT + 1 + i * elementSize,
					124, 20, "+- Amount");
			operation0List[i].packedFGColour = buttonYESColor;
			buttonList.add(operation0List[i]);
			tfid++;
		}
		operation1List = new GuiButton[list.length];
		for (int i = 0; i < operation1List.length; i++) {
			operation1List[i] = new GuiButton(tfid, width / 2 + 125,
					25 + fontRendererObj.FONT_HEIGHT + 1 + i * elementSize, 124, 20, "+- Amount % (additive)");
			operation1List[i].packedFGColour = buttonNOColor;
			buttonList.add(operation1List[i]);
			tfid++;
		}
		UUIDLeast = new GuiTextField[list.length];
		for (int i = 0; i < amountList.length; i++) {
			UUIDLeast[i] = new GuiTextField(tfid, fontRendererObj, width / 2 - 80,
					2 + 45 + 2 * (fontRendererObj.FONT_HEIGHT + 1) + i * elementSize, 75, 16);
			UUIDLeast[i].setText(String.valueOf(Integer.valueOf((int) (1000000.0 * Math.random()))));
			tfid++;
		}
		UUIDMost = new GuiTextField[list.length];
		for (int i = 0; i < amountList.length; i++) {
			UUIDMost[i] = new GuiTextField(tfid, fontRendererObj, width / 2 + 125,
					2 + 45 + 2 * (fontRendererObj.FONT_HEIGHT + 1) + i * elementSize, 75, 16);
